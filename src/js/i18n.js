...
const dict = {
  ...
  zh: {
    ...
    home_sandbox: '🔬 自由实验',
    journey_prog: '{0}/{1}',
    journey_complete: '🎉 这一程全部完成！',
    continue_journey: '继续冒险',
    start_journey: '开始冒险',
    load_custom:  '载入自定义等式',
    random_btn:   '随机来一题',
    theme_toggle: '切换主题',
    ...
    design_note_2:'看起来像“变号”，本质上是两边做了同样的事。',
    design_note_3:'括号题的重点，是看清外面的倍数怎样分到里面每一项。',

    /* 新手引导 */
    onboard_title:   '欢迎来到等式实验室！',
    onboard_sub:     '三步学操作，然后就能自己动手啦',
    onboard_s1:      '<strong>拖卡片</strong> — 按住数字 / 字母卡片，拖到等号另一侧。这相当于两边做同样的调整，所以另一侧会出现对应的相反项。',
    ...
    status_reset_m: '拖动卡片试一试。先想想哪一步能让 △ 更孤单；需要时也可以用两边同时操作或展开括号。',
    ...
    number_block:   '数量块',
    expand_btn:     '展开',
    expand_hint:    '拖进括号只是快捷操作，更基础的方法是先展开再整理',
    empty_group:    '把卡片拖到这里时，系统会自动换成括号里对应的一份。',
    empty_side:     '这一边现在空了，等价于 0。',
    bracket_done:   '括号已展开',

    /* Smart Actions */
    move_to:       '把 {0} 移到{1}',
    move_note:     '先把 △ 身边的常数请走，并记得两边要保持一样多。',
    ...
    coach_group:      '这一关有倍数括号。更稳的做法是先点“展开”，看清外面的倍数怎样分到里面每一项。',
    coach_expand:     '如果你想看清乘法怎样作用到里面每一项，可以先展开，再决定下一步要移走谁。',
    ...
    rand_bracket_s: '可以比较两条路线：先展开，或者先整理外面的常数。',
  ...
  ja: {
    ...
    home_sandbox: '🔬 自由実験',
    journey_prog: '{0}/{1}',
    journey_complete: '🎉 ぜんぶクリア！',
    continue_journey: '続きへ',
    start_journey: 'ぼうけん開始',
    ...
    design_note_2:'見た目は「符号が変わる」けれど、本当は両側に同じことをしているんだよ。',
    design_note_3:'かっこの問題で大事なのは、外の数が中のそれぞれにどうかかるかを見ること。',

    /* はじめてガイド */
    onboard_title:   '等式ラボへようこそ！',
    onboard_sub:     '3つのステップでやり方をおぼえよう',
    onboard_s1:      '<strong>カードをドラッグ</strong> — 数字や △ のカードをおして、等号の反対がわへ動かそう。これは両側に同じ調整をするのと同じだから、反対側では対応する反対の項として見えるよ。',
    ...
    status_reset_m: 'カードをドラッグしてみよう。まずは △ をひとりに近づける一手を考えよう。必要なら両側いっしょの操作や、かっこをひらくことも使えるよ。',
    ...
    number_block:   '数ブロック',
    expand_btn:     'ひらく',
    expand_hint:    '中に入れるのは近道。まずは「ひらく」で考えるとわかりやすいよ',
    empty_group:    'ここへ入れると、外の数に合う1つ分の形に自動で直すよ。',
    empty_side:     'このがわはからっぽ（= 0）。',
    bracket_done:   'かっこをひらいたよ',

    /* 次にやること */
    move_to:       '{0} を{1}にうつす',
    move_note:     'まず △ のとなりの数を反対がわへ。両側が同じだけのままになることを忘れないでね。',
    ...
    coach_group:      'この問題には倍のかっこがあるよ。まず「ひらく」をおして、外の数が中のそれぞれにどうかかるか見てみよう。',
    coach_expand:     'かけ算が中のそれぞれにどうかかるか見たいなら、先にひらいてから次の一手を考えよう。',
    ...
    rand_bracket_s: 'ひらいてから整理するやり方と、外の数を先に見るやり方をくらべてみよう。',
  ...
  en: {
    ...
    design_note_2:'It may look like “the sign flips,” but really both sides are being changed in the same way.',
    design_note_3:'For bracket problems, the key idea is how the outside multiplier is distributed to every term inside.',

    /* Onboarding */
    onboard_title:   'Welcome to Equation Lab!',
    onboard_sub:     'Learn in 3 steps, then try it yourself',
    onboard_s1:      '<strong>Drag a card</strong> — Hold a number or variable card and drag it to the other side. This matches making the same change to both sides, so the other side shows the corresponding opposite term.',
    ...
    status_reset_m: 'Try dragging a card. First ask which move makes x more alone; when needed, use an operation on both sides or expand a bracket.',
    ...
    expand_btn:     'Expand',
    expand_hint:    'Dragging into a bracket is a shortcut; the more basic idea is to expand first, then simplify',
    empty_group:    'Drag a card here and the app will convert it into the matching inside-the-bracket amount.',
    empty_side:     'This side is now empty (= 0).',
    bracket_done:   'Bracket expanded',

    /* Smart Actions */
    move_to:       'Move {0} to the {1}',
    move_note:     'Clear the constants next to x first, while keeping both sides equal.',
    ...
    coach_group:      'This problem has a multiplier bracket. The safer route is to expand first and see how the outside multiplier affects each inside term.',
    coach_expand:     'Want to see how multiplication distributes? Expand first, then decide what to clear next.',
    ...
    rand_bracket_s: 'Compare two routes: expand first, or tidy up the outside constants first.',
