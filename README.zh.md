...
等式实验室是一款面向小学高年级学生的互动数学网站。孩子可以通过拖拽、整理、展开、“两边同时操作”和解释题意，逐步建立等式变换与代数直觉。
...
- `学习地图`：把关卡按学习目标组织成更像游戏旅程的入口
- `等式闯关`：循序练习平衡、逆运算、分数、括号和应用题
- `算数基础能力训练区`：按能力域、专项和阶段训练数感、四则关系、乘法结构、因数倍数、量与单位和综合挑战
- `自由实验`：可自由输入方程、生成随机题、比较不同解法
...
- 使用 `+`、`-`、`×`、`÷` 工具对两边同时操作
- 支持整数、简单分数、括号、应用题等内容
- 通过关卡前的小判断，减少“不读题直接乱移项”
- 把算数基础训练放在独立区域，避免主线变成泛刷题站
- 先用选择/配对题找结构感觉，再进入固定 10 题训练；完成整局后才结算本地硬币
- 提供中文、日语、英语三语界面与课程文本
- 可作为静态网站部署
...
我们的目标不是把孩子训练成“会移项”的做题机器，而是帮助他们理解：只要两边一起做同样的事，等式就会继续成立；计算技巧也不是魔法，而是来自数与式子的结构。
